...
`supabase/functions/scrape-activities/`: `index.ts` (the Edge Function) and **`sources.json` — the list of listing pages it reads. That file is the one place sources live.** Firecrawl extracts every club/course/show/event from each page; rows whose ages overlap 12–18 are upserted into `activities` on `source_url` (the item's own page).

- **Schedule:** `pg_cron` job `scrape-activities-batch`, `*/3 3 * * 0` — every 3 minutes, 03:00–03:57 UTC, **Sundays only**. One source per call (a listing extraction can take a minute; the function limit is 150 s); calls after the last source run cleanup. Progress per source is in `scrape_runs` (`last_status`, `found`/`kept`/`inserted`/`updated`, `skipped` reasons, `last_error`).
- **Why weekly:** each listing extraction costs **5 Firecrawl credits**; 17 sources = 85 per pass. The plan has **1,000 credits/month** (renews on the 22nd) — daily would need ~2,600. Weekly is ~370/month. Change the cron expression only after checking the plan.
- **Auth:** every call needs header `x-scraper-secret` equal to the `SCRAPER_SECRET` function secret. The cron reads the same value from Vault (`vault.decrypted_secrets`, name `scraper_secret`), so it is never in this public repo. To rotate: `supabase secrets set SCRAPER_SECRET=<new>` **and** `select vault.update_secret((select id from vault.secrets where name='scraper_secret'), '<new>')` — change only one and every run gets 401.
- **Check it:** `supabase db query --linked "select * from scrape_runs order by last_run_at desc"`; cron history in `cron.job_run_details`; the function's real reply in `net._http_response` (the 150 s timeout means it is actually recorded now).
- **Run by hand** (needs the secret): `POST .../functions/v1/scrape-activities` → next unrun source; `?source=<index>&dry=1` → extract one source, write nothing, and report remaining credits; `?recheck=1` → re-apply the rules to every stored row, costs no credits (use after changing the rules).
- **Adding a source:** open the page, confirm it lists real current activities for 12–18 in Jerusalem, add it to `sources.json` (`page_ages` only if the whole page is a youth section and items have no ages), `supabase functions deploy scrape-activities`, then `?source=<n>&dry=1` and read what it kept before the next Sunday run writes it.
- **The model is not trusted with facts the title states.** Seen on real rows: "ד-ה" (grades 4–5, ages 9–10) stored as 12–18, "ב+ד" (Mon+Wed) stored as Sun+Tue, "00:00" as a placeholder time, a five-day run when the days were unknown, "יוגה נשים" returned as a teen activity, an unchanged page giving a different `age_max` hours apart. `refine()` therefore parses grades, ages and days from the title itself and overrides the model; drops "00:00" and 4+-day runs; rejects adult-only titles, article titles, and titles naming a past year; and sends every dance/theater title to `stage`. A model price of 0 is treated as unstated.
- **Most listing pages have no days/hours** — those live on each club's own page. Such rows are kept with the schedule empty and the app shows "המועדים באתר המארגן". Scraping each club page would fill them but costs a credit per club per run.
- **Long pages come back incomplete.** Beit HaKerem's listing is ~170k characters and yields 4–5 teen items per run, fewer than the page has. Cleanup only unpublishes a row after its source succeeded while the row went unseen for 15 days (two weekly runs), so one incomplete run does not hide anything.
- **Not scrapeable:** `jerusalem.muni.il` (every Firecrawl engine fails, re-confirmed 2026-09-23) — which is also where "בירת הנוער" is published. Removed after testing: גינות העיר (its youth page is news articles, not activities).
