...
- **Guest mode** — keeps working **while the backend is down**, provided the device has connected successfully once: the activity list is read from the database and the last good copy is stored locally, and guest prefs are localStorage. A device that has *never* connected and hits a paused backend gets a notice card explaining that, with a retry button — not an empty-looking catalog. (Before 2026-09-23 the list was a hardcoded array, so this worked even on a device's first visit; that array could not follow the scraper, so it was retired.)
...
- **4 of the 22 activities are real**, and as of 2026-09-22 three of them (the Beit Hakerem ones) are kept fresh automatically — see the scraper section below. The fourth (Western Wall Tunnels) is still a one-time manual entry; the municipality's own site blocks the scraper.
...
`supabase/functions/scrape-activities/` (Deno Edge Function) + `pg_cron` (migration `20260922150000_schedule_activity_scraper.sql`, job name `scrape-activities-daily`, `0 4 * * *` — 04:00 UTC daily) + Firecrawl for the actual page reading. Requires the `FIRECRAWL_API_KEY` secret (already set).

- **Check it's still scheduled:** `supabase db query --linked "select jobid, jobname, schedule, active from cron.job"`.
- **Run it manually / check what it does right now** without waiting for the schedule:
  ```
  curl -X POST "https://soncpzmasqcjxcawhqqw.supabase.co/functions/v1/scrape-activities" \
    -H "Authorization: Bearer sb_publishable_Js1OgoSr1x7INhSfR_hfwQ_0aTH1vIW"
  ```
  Returns a per-source result: `ok` (upserted), `skipped` (with a `reason` — no usable schedule, or age doesn't overlap 12–18), or `error`.
- **Growing `SOURCES` in `index.ts` is manual and reviewed, by design** — open the candidate page yourself, confirm it's a real currently-offered activity for roughly 12–18, then add it and redeploy (`supabase functions deploy scrape-activities`). It does not crawl for new URLs on its own; see CLAUDE.md for why.
- **The municipality's own jerusalem.muni.il pages are not scrapeable** — Firecrawl's `SCRAPE_ALL_ENGINES_FAILED` even in stealth mode, re-confirmed 2026-09-23 on `/he/residents/youth/` and `/he/residents/youth/youth-movements/` (plain requests get a 403 "Access Denied" page). Don't burn time re-adding them to `SOURCES` without first confirming that's changed. **"בירת הנוער" has no site of its own** — it is the municipality's July–August youth festival, published on that same blocked site and on Facebook. The only reachable third-party page about it (allaboutjerusalem.com) scraped cleanly and accurately, but describes a 2011 event.
- **LLM extraction is not stable between runs of an unchanged page — including structured fields.** One real run substituted "עין כרם" (Ein Kerem) for "בית הכרם" (Beit Hakerem) in description prose before the schema was tightened to demand verbatim wording. On 2026-09-23 two runs four hours apart against the *same unchanged* D&D page gave `age_max` 17, then 16, and two different titles; the other two pages changed only their `short_description`. Each daily run can therefore rewrite facts, and since the app now reads the table directly, those changes reach users. The fix worth building is to hash the page's markdown and skip extraction when it hasn't changed — which also stops spending credits on pages that didn't move.
- **The cron cannot see whether a scrape worked.** `pg_net` gives up after its 5 s default (`net._http_response` shows `timed_out = true` for the 2026-09-23 run) while the function needs ~5 s per source. The function keeps running server-side and does write — `updated_at` on all three rows lands at 04:00:08–18 — but `cron.job_run_details` reports `succeeded` merely because the request was *sent*, so a scrape that fails outright would look identical. Passing `timeout_milliseconds` to `net.http_post` would make the real result visible. Also: at ~5 s per source, sequential, the function's wall-clock limit caps `SOURCES` somewhere around 25–30 before a run gets cut off mid-list.
