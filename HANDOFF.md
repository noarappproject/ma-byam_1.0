...
- **The municipality's own jerusalem.muni.il pages are not scrapeable** — Firecrawl's `SCRAPE_ALL_ENGINES_FAILED` even in stealth mode. Don't burn time re-adding them to `SOURCES` without first confirming that's changed.
- **LLM extraction can misfire even on a page it reads successfully.** One real run substituted "עין כרם" (Ein Kerem) for "בית הכרם" (Beit Hakerem) in generated description prose before the schema was tightened to demand verbatim wording. Structured fields (schedule, price, age) have held up in testing; free-text prose from a fresh source is worth a read before you trust it blindly.
- **Not yet built:** unpublishing a row whose source activity disappears (today it just stops refreshing, silently), and a live JS-mirror sync — `docs/index.html`'s hardcoded `activities` array does **not** update itself when the scraper changes a DB row. The 3 automated rows were hand-synced once, on 2026-09-22; a future scraper run that changes their price/schedule again will update the database but not the array shown on the live site until someone re-syncs it by hand, or the array is retired in favor of a real query (see CLAUDE.md's open note on that).
